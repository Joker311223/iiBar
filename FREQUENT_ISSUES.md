...
- [iiBar removed an item](#iibar-removed-an-item)
- [Layout changes on its own](#layout-changes-on-its-own)
- [How do I solve the `iiBar cannot arrange menu bar items in automatically hidden menu bars` error?](#how-do-i-solve-the-iibar-cannot-arrange-menu-bar-items-in-automatically-hidden-menu-bars-error)
...
- [Hidden items still visible when the iiBar bar is off](#hidden-items-still-visible-when-the-iibar-bar-is-off)
...
macOS inserts new status items at the **far left** of the menu bar. iiBar treats that region as the **Hidden** section (or **Always Hidden** when that section is enabled). Some apps do not persist their menu bar position across relaunch, so iiBar may treat them as newly detected items even after you have moved them before.

iiBar 2.0 remembers layout through **profiles** and the **New Items** badge in **Settings → Menu Bar Layout**. Move that badge to choose where newly detected items should appear.
...
## iiBar removed an item

iiBar cannot delete menu bar items. An item that seems to have vanished was usually moved into the **Hidden** or **Always Hidden** section by macOS or by your layout.

**Workaround:**

1. **Option + click** the iiBar icon to reveal the always-hidden section (or double-click an empty area of the menu bar if you enabled that in **Settings → Advanced**).
2. **⌘ Command + drag** the item into a different section.

## Layout changes on its own

iiBar **does** persist item order per profile. Layout drift usually comes from one of these causes:
...
- A **spacing change** that requires iiBar to relaunch apps with menu bar items.

When display spacing must be applied across a transition, iiBar may relaunch affected apps. That can look like icons jumping or duplicating briefly. Enable **Confirm before relaunching apps** in **Settings → Displays** if you want a prompt first.
...
## How do I solve the `iiBar cannot arrange menu bar items in automatically hidden menu bars` error?

macOS does not expose enough of the menu bar for iiBar to rearrange items while **Automatically hide and show the menu bar** is active.
...
Some apps draw menu bar icons outside the normal status-item APIs iiBar enumerates, or host their icon under the `com.apple.controlcenter` namespace without a stable identifier. iiBar may show the icon in the menu bar but not list it by name in **Settings → Menu Bar Layout**.
...
Little Snitch is the most frequently reported third-party menu bar app in iiBar issues. Its agent (`at.obdev.littlesnitch.agent`) often appears at the Accessibility layer as `com.apple.controlcenter:Item-0` **without a stable source PID**, so iiBar must identify it indirectly (marker-pair resolution) rather than by bundle ID alone.

This causes several confusing symptoms that look like iiBar bugs but stem from how Little Snitch hosts its status item on macOS 26:
...
| Clicking the icon while hidden makes the iiBar icon follow the cursor | [#332](https://github.com/stonerl/Thaw/issues/332) |
...
- iiBar maintainers are in contact with the Little Snitch developers about the underlying identification problem.

**Workarounds to try**

1. **Update iiBar** to the latest 2.0 beta (or stable, whichever is newer).
2. If the icon vanishes when iiBar launches, **quit iiBar**, **⌘ Command + drag** the Little Snitch icon to the far right of the menu bar, then relaunch iiBar ([#709](https://github.com/stonerl/Thaw/issues/709)).
3. Clear iiBar's cache and reset Accessibility permission, then re-grant when prompted ([#643](https://github.com/stonerl/Thaw/issues/643)):

   ```sh
   rm -rf ~/Library/Caches/com.joker311223.iiBar
   tccutil reset Accessibility com.joker311223.iiBar
   ```

4. If Little Snitch **Network Monitor meters** keep returning to Always Hidden, try placing them from Little Snitch's own preferences rather than only through iiBar ([#575](https://github.com/stonerl/Thaw/issues/575)).
...
- Moving Little Snitch **only** inside iiBar's Layout panel may not stick; menu-bar **⌘ Command + drag** (with iiBar quit, if needed) is more reliable for some users.

If none of the above helps, file a bug with diagnostic logs. Search the log for `obdev`, `littlesnitch`, and `marker-pair` — absence of `at.obdev.littlesnitch.agent` usually means iiBar has not identified the item yet.

## CodexBar

CodexBar issues are **upstream**: certain CodexBar versions wrote state into macOS Control Center preferences that caused iiBar (and other menu bar managers) to misplace the icon into Hidden or Always Hidden ([#605](https://github.com/stonerl/Thaw/issues/605)).
...
iiBar versions seen in reports: **1.2.0** (original report), **2.0.0-beta.13+** (follow-up testing). The fix is on the CodexBar side; use a current iiBar build together with CodexBar **0.30.1 or later**.
...
iiBar may **relaunch apps with menu bar items** when a display transition requires applying different menu bar spacing. That can produce duplicate icons if the host app also relaunches its agent — the duplicate usually belongs to the app, not iiBar.
...
3. After wake from sleep, quit and reopen iiBar if layout looks stale.
...
iiBar uses **Screen Recording** for layout thumbnails, tooltips, and some overlay features. Without it, **Settings → Menu Bar Layout** may fail to load items, and revealing only the always-hidden section can be limited ([#628](https://github.com/stonerl/Thaw/issues/628)).

Grant permissions under **Settings → Advanced → Permissions**. If macOS keeps re-prompting after reboot despite approval ([#683](https://github.com/stonerl/Thaw/issues/683)), remove iiBar from **System Settings → Privacy & Security → Screen Recording**, then add it again.

## Hidden items still visible when the iiBar bar is off

Disabling **Use iiBar Bar** stops iiBar from actively concealing items. Icons that were hidden while the bar was enabled may remain visible until iiBar manages the layout again.

**Workaround:** Turn **Use iiBar Bar** back on, or open **Settings → Menu Bar Layout** and re-apply your sections.

Related report: [#610](https://github.com/stonerl/Thaw/issues/610).

## Flickering, refreshing, or "dancing" icons

Frequent redraws are often caused by the **host app** updating its status item (for example Shazam or Amphetamine) rather than by iiBar. Mission Control and space switches can also trigger full layout storms ([#718](https://github.com/stonerl/Thaw/issues/718)).

**Workarounds:**

1. Update iiBar to the latest release.
2. Check whether the affected app has a menu-bar redraw setting.
3. If flicker started after a iiBar update, file a bug with logs — it may be a regression.
...
1. Confirm you are on the **latest iiBar release** and **macOS 26+** (the active target for current development).
...
**Support policy:** iiBar versions below **1.2.0** on macOS versions below **15.7.7** are no longer supported. macOS 26 and later are actively supported.
