...
enum MarkerPairResolver {
    ...
    struct Marker: Equatable {
        ...
        /// resolves to a bundle ID that is not Control Center or iiBar.
        let owningPID: pid_t?
    }
    ...
    /// are skipped to prevent misattribution. iiBar and Control Center
    ...
    ///     items-only list. Callers are expected to pre-filter iiBar
    ///     control items and the iiBar self-registration window.
    ///   - iibarBundleID: iiBar's own bundle identifier; excluded from
    ...
    /// (bundle-identifier shape), is not a iiBar control item, and is
    /// not the iiBar self-registration window.
    static func extractMarkers(
        from windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)],
        iibarControlItemPrefix: String,
        iibarBundleID: String
    ) -> [Marker] {
        windows.compactMap { window in
            guard let title = window.title, title.contains(".") else { return nil }
            if title.hasPrefix(iibarControlItemPrefix) { return nil }
            if title == iibarBundleID { return nil }
            return Marker(
