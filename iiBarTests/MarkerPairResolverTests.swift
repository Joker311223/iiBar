...
final class MarkerPairResolverTests: XCTestCase {
    ...
    private let iibarBundleID = "com.joker311223.iiBar"
    private let ccBundleID = "com.apple.controlcenter"
    private let iibarControlItemPrefix = "iiBar.ControlItem."
    ...
    /// Marker's CG owner is the widget's real app (not CC, not iiBar):
    ...
    /// Marker's CG owner resolves to iiBar itself: rejected, falls
    /// through to title lookup. The title-lookup result must also
    /// be checked for iiBar self-attribution (see the next test).
    func testiiBarOwnerFallsThroughToTitleLookup() {
        let icons = [icon(windowID: 1, title: "Item-0")]
        ...
        let result = MarkerPairResolver.resolve(
            ...
            ccBundleID: ccBundleID,
            pidToBundleID: { pid in
                if pid == 100 { return self.iibarBundleID }
                if pid == 777 { return "com.example.widget" }
                return nil
            },
            bundleIDToPID: { bundleID in
    ...
    /// Both paths resolve to iiBar: no resolution emitted. Defensive
    /// guarantee that iiBar's own PID is never attributed to a
    /// third-party widget regardless of where the lookup happens to
    /// land.
    func testBothPathsResolveToiiBarProducesNoResult() {
        let icons = [icon(windowID: 1, title: "Item-0")]
        let markers = [marker(windowID: 2, title: "com.example.widget", owningPID: 100)]
        let result = MarkerPairResolver.resolve(
            unresolvedIcons: icons,
            ...
            iibarBundleID: iibarBundleID,
            ccBundleID: ccBundleID,
            pidToBundleID: { _ in self.iibarBundleID },
    ...
    func testExtractMarkersExcludesGenericTitles() {
        ...
        let markers = MarkerPairResolver.extractMarkers(
            from: windows,
            iibarControlItemPrefix: iibarControlItemPrefix,
            iibarBundleID: iibarBundleID
        )
        XCTAssertEqual(markers.map(\.windowID), [4])
    }

    /// iiBar control items are excluded by the iiBar.ControlItem.
    /// prefix even though their titles contain dots.
    func testExtractMarkersExcludesiiBarControlItems() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "iiBar.ControlItem.Hidden", CGSize(width: 5016, height: 33), nil),
            (2, "iiBar.ControlItem.AlwaysHidden", CGSize(width: 5016, height: 33), nil),
    ...
    /// The iiBar self-registration window (title equals the iiBar bundle
    /// identifier) is excluded so iiBar's own PID can never be
    /// attributed to a third-party widget via the title-lookup path.
    func testExtractMarkersExcludesiiBarSelfRegistration() {
        let windows: [(windowID: CGWindowID, title: String?, size: CGSize, owningPID: pid_t?)] = [
            (1, "com.joker311223.iiBar", CGSize(width: 33, height: 33), nil), // iiBar self
