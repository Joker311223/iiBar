...
/// (1) iiBar icon, (2) non-hideable system item, (3) new hideable item,
...
final class PlanLeftmostMoveTests: XCTestCase {
    ...
    /// The iiBar visible-control icon left of the divider triggers the
    /// iiBar-icon recovery branch.
    func testiiBarIconLeftOfDividerTriggersiiBarIconBranch() {
        let iibar = leftmostItem(
        ...
        let decision = LayoutSolver.planLeftmostMove(
            ...
            observation: LayoutSolver.LeftmostObservation(
                ...
                sectionByWindowID: [iibar.windowID: .hidden],
                previousWindowIDs: []
            ),
            savedSectionOrder: [:],
        ...
        if case let .iibarIcon(item) = decision {
            XCTAssertEqual(item.windowID, 700)
        } else {
            XCTFail("expected .iibarIcon, got \(decision)")
        }
    }
