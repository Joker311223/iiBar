...
struct AutomationSettingsPane: View {
        ...
        IceSection(options: [.isBordered]) {
            VStack(alignment: .leading, spacing: 12) {
                Toggle("Enable Settings URI Scheme", isOn: $settings.isSettingsURIEnabled)
                    .annotation("Allow external applications to read and modify \(Constants.displayName) settings via iibar:// URLs.")
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack(alignment: .top, spacing: 8) {
                        Text(verbatim: "1.")
                        Text("When an app sends a iibar:// URL to change settings, iiBar checks if that app is whitelisted.")
                    }
    ...
    private var envVarsHelp: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(verbatim: "IIBAR_HOOK_PHASE, IIBAR_HOOK_SCOPE, IIBAR_PROFILE_ID, IIBAR_PROFILE_NAME, IIBAR_PREVIOUS_PROFILE_ID, IIBAR_PREVIOUS_PROFILE_NAME")
                .font(.caption.monospaced())
                .foregroundStyle(.secondary)
                .textSelection(.enabled)
                .fixedSize(horizontal: false, vertical: true)

            Text("Example: a bash pre-hook could `defaults write com.bjango.istatmenus5 ActiveProfile -string \"$IIBAR_PROFILE_NAME\"` to keep iStat Menus in sync with iiBar.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .textSelection(.enabled)
                .fixedSize(horizontal: false, vertical: true)
