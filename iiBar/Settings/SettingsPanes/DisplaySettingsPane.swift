...
struct DisplaySettingsPane: View {
    ...
    var body: some View {
        IceForm {
            ...
            CalloutBox(systemImage: "exclamationmark.triangle.fill", font: .callout, foregroundStyle: Color.warning) {
                Text("When a display transition requires iiBar to apply a different menu bar spacing, iiBar relaunches apps with menu bar items. Relaunching apps may cause unsaved input, progress, or transient app state to be lost.")
            }
    ...
    private var confirmSpacingRelaunchControls: some View {
        Toggle("Confirm before relaunching apps", isOn: $displaySettings.confirmSpacingRelaunch)
            .annotation("Before a display change or spacing edit relaunches your menu bar apps, iiBar asks you to confirm. Turn this off to apply spacing changes and relaunch apps without confirmation.")
    ...
    private func globalConfirmationMessage(for pending: PendingGlobalApply) -> String {
        let profileName = pending.activeProfileName ?? ""
        let displayMessage = String(localized: "This will overwrite the settings of \(pending.displayCount) display with the global template. If the active display's spacing changes, iiBar will relaunch each app with a menu bar item. Relaunching apps may cause unsaved input, progress, or transient app state to be lost.")
        if pending.activeProfileID != nil {
