...
final class ProfileManager: ObservableObject {
    ...
    init(profilesDirectory: URL? = nil) {
        ...
        if let profilesDirectory {
            ...
            self.profilesDirectory = appSupport
                .appendingPathComponent("iiBar/Profiles", isDirectory: true)
        }
    ...
    func performSetup(with appState: AppState) {
        ...
        DistributedNotificationCenter.default()
            .publisher(for: Notification.Name("com.joker311223.iiBar.focusFilterActivated"))
        ...
        DistributedNotificationCenter.default()
            .publisher(for: Notification.Name("com.joker311223.iiBar.focusFilterDeactivated"))
            .debounce(for: .seconds(0.5), scheduler: DispatchQueue.main)
            .sink { [weak self] _ in
                guard let self else { return }
                Task { [weak self] in
        ...
        Task { [weak self] in
            guard let self else { return }
            do {
                let current = try await IIBarFocusFilter.current
    ...
    /// IIBAR_PREVIOUS_PROFILE_ID env var.
    func applyProfile(
        _ profile: Profile,
