...
/// Per-display configuration for the iiBar Bar.
struct DisplayIceBarConfiguration: Codable, Equatable {
    /// Whether the iiBar Bar is enabled on this display.
    let useIceBar: Bool

    /// The location where the iiBar Bar appears on this display.
    ...
    /// The layout mode for the iiBar Bar on this display.
    let iceBarLayout: IceBarLayout

    /// The maximum number of items per row when the iiBar Bar is in grid layout.
    ///
