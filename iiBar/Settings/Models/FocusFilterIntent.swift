...
/// An entity representing an iiBar profile, exposed to the system
...
struct ProfileEntityQuery: EntityQuery {
    ...
    private func allProfiles() -> [ProfileEntity] {
        ...
        let manifestURL = appSupport
            .appendingPathComponent("iiBar/Profiles/profiles.json")
...
/// Focus Filter that applies an iiBar profile when a Focus mode activates.
/// Appears in System Settings → Focus → [mode] → Focus Filters → Add "iiBar".
struct IIBarFocusFilter: SetFocusFilterIntent {
    ...
    static nonisolated(unsafe) var description: IntentDescription? = IntentDescription(
        "Apply an iiBar menu bar profile when this Focus activates.",
    ...
    func perform() async throws -> some IntentResult {
        guard let profile,
            ...
            DistributedNotificationCenter.default().postNotificationName(
                Notification.Name("com.joker311223.iiBar.focusFilterDeactivated"),
                object: nil,
        ...
        DistributedNotificationCenter.default().postNotificationName(
            Notification.Name("com.joker311223.iiBar.focusFilterActivated"),
            object: nil,
