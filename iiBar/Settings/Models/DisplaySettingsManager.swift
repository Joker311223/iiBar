...
/// Manages per-display iiBar Bar configuration.
///
...
final class DisplaySettingsManager: ObservableObject {
    ...
    /// Whether iiBar asks for confirmation before a spacing change relaunches
    ...
    /// Internal access so unit tests in iiBarTests can seed and assert it.
    ...
    /// translate on-disk system spacing into iiBar's relative offset model.
    ...
    /// connected display so iiBar does not overwrite a user's manual
    ...
    /// When the user has manually set NSStatusItemSpacing outside of iiBar
    ...
    /// consulted because iiBar drives both keys from a single offset; users
    ...
    private func presentSpacingRelaunchConfirmation() -> Bool {
        ...
        alert.informativeText = String(localized: "When a display transition requires iiBar to apply a different menu bar spacing, iiBar relaunches apps with menu bar items. Relaunching apps may cause unsaved input, progress, or transient app state to be lost.")
    ...
    /// Whether any connected display has the iiBar Bar enabled.
    ...
    /// Toggles the iiBar Bar for the display with the active menu bar.
    func toggleIceBarForActiveDisplay() {
        guard let uuid = Bridging.getActiveMenuBarDisplayUUID() else {
            diagLog.warning("Cannot toggle iiBar Bar — no active menu bar display UUID")
            return
        }
        updateConfiguration(forDisplayUUID: uuid) { config in
