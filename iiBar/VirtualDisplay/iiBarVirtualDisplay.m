...
void *iiBarVirtualDisplayCreate(void) {
    Class descriptorClass = NSClassFromString(@"CGVirtualDisplayDescriptor");
    Class settingsClass = NSClassFromString(@"CGVirtualDisplaySettings");
    Class modeClass = NSClassFromString(@"CGVirtualDisplayMode");
    Class displayClass = NSClassFromString(@"CGVirtualDisplay");
    if (!descriptorClass || !settingsClass || !modeClass || !displayClass) {
        NSLog(@"[iiBarVirtualDisplay] CGVirtualDisplay classes unavailable; cannot create");
        return NULL;
    }
    ...
    @try {
        ...
        descriptor.name = @"iiBar Resolver";
        ...
        if (!display) {
            NSLog(@"[iiBarVirtualDisplay] initWithDescriptor returned nil");
            return NULL;
        }
        ...
        CGVirtualDisplayMode *mode = [[modeClass alloc] initWithWidth:width
                                                          ...
                                                          refreshRate:60.0];
        if (!mode) {
            NSLog(@"[iiBarVirtualDisplay] mode init returned nil");
            return NULL;
        }
        ...
        if (![display applySettings:settings]) {
            NSLog(@"[iiBarVirtualDisplay] applySettings failed");
            return NULL;
        }

        // Retain across the C boundary; iiBarVirtualDisplayDestroy releases it,
        // and deallocating the CGVirtualDisplay removes the display.
        return (void *)CFBridgingRetain(display);
    } @catch (NSException *exception) {
        NSLog(@"[iiBarVirtualDisplay] creation raised %@: %@", exception.name, exception.reason);
        return NULL;
    }
}

uint32_t iiBarVirtualDisplayGetID(void *handle) {
    if (!handle) {
        return 0;
    }
    @try {
...
void iiBarVirtualDisplayDestroy(void *handle) {
    if (!handle) {
        return;
    }
