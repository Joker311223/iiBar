...
/// iiBarVirtualDisplayDestroy to remove the display.
void *_Nullable iiBarVirtualDisplayCreate(void);

/// The CGDirectDisplayID of a handle created by iiBarVirtualDisplayCreate, or
/// 0 when the handle is NULL.
uint32_t iiBarVirtualDisplayGetID(void *_Nullable handle);

/// Releases the handle and removes the virtual display.
void iiBarVirtualDisplayDestroy(void *_Nullable handle);
