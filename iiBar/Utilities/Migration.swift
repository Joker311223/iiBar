...
// MARK: - Migrate Per-Display iiBar Bar

extension MigrationManager {
    /// Migrates legacy global iiBar Bar settings to per-display configurations.
    private func migratePerDisplayIceBar() -> MigrationResult {
        ...
        // Only create per-display configs if the user had iiBar Bar enabled.
        guard useIceBar else {
            Defaults.set(true, forKey: .hasMigratedPerDisplayIceBar)
            diagLog.info("Per-display iiBar Bar migration: iiBar Bar was disabled, nothing to migrate")
            return .success
        }
        ...
        do {
            ...
            Defaults.set(true, forKey: .hasMigratedPerDisplayIceBar)
            diagLog.info("Per-display iiBar Bar migration: migrated \(configs.count) display(s)")
        } catch {
    ...
    enum MigrationError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case let .perDisplayIceBarMigrationError(error):
                "Error migrating per-display iiBar Bar configuration: \(error)"
