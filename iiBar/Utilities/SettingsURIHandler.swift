...
/// Handles settings manipulation via iibar:// URLs with whitelist-based security.
@MainActor
enum SettingsURIHandler {
    ...
    /// Handles iibar://set?key=X&value=Y&type=bool URL.
...
extension Notification.Name {
    ...
    static let settingsDidChangeViaURI = Notification.Name("com.joker311223.iiBar.settingsDidChangeViaURI")

    /// Posted when a per-display setting is changed externally via Settings URI scheme.
    static let perDisplaySettingsDidChangeViaURI = Notification.Name("com.joker311223.iiBar.perDisplaySettingsDidChangeViaURI")

    /// Posted when a get request response is broadcast via distributed notification.
    static let settingsURIGetResponse = Notification.Name("com.joker311223.iiBar.settingsURIGetResponse")

    /// Posted when the Settings URI whitelist changes.
    static let settingsURIWhitelistDidChange = Notification.Name("com.joker311223.iiBar.settingsURIWhitelistDidChange")
}
