...
struct IceSettingsImporter {
    ...
    private func importGeneralSettings(from iceSettings: [String: Any]) -> Int {
        ...
        let mappings: [(Defaults.Key, String)] = [
            ...
            // Legacy iiBar Bar keys kept for migration compatibility
            (.useIceBar, "UseIceBar"),
        ...
        // Generate per-display configurations when importing iiBar Bar settings
        imported += importPerDisplayIceBarSettings(from: iceSettings)

        return imported
    }

    /// Generates per-display iiBar Bar configurations from imported Ice settings.
    private func importPerDisplayIceBarSettings(from iceSettings: [String: Any]) -> Int {
        ...
        do {
            ...
            diagLog.info("Generated per-display iiBar Bar configs for \(configs.count) display(s) from Ice import")
            return 1
        } catch {
            diagLog.error("Failed to encode per-display iiBar Bar configs during import: \(error)")
            return 0
        }
    }
