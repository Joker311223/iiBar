...
final class MenuBarSection {
    ...
    /// A Boolean value that indicates whether the iiBar Bar should be used
    ...
    /// The best screen to show the iiBar Bar on.
    ///
    ...
    func show(triggeredByHotkey: Bool = false) {
        ...
        // Determine whether we should use the iiBar Bar based on settings.
        ...
        if shouldUseIceBarBasedOnSettings {
            ...
            case .iceBar:
                diagLog.info("Not enough space to show items inline, falling back to iiBar Bar")
            }
        } else {
            preferredPresentationMode = .inline
        }

        // Use Ice iiBar if settings say so OR if items still won't fit inline.
        ...
        // If we made it here, we're not using the iiBar Bar.
    ...
    func hide() {
        ...
        menuBarManager.iceBarPanel.close() // Make sure iiBar Bar is always closed.
