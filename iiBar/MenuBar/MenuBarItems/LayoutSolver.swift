...
enum LayoutSolver {
    ...
    enum LeftmostMove: Equatable {
        /// The iiBar visible-control icon is sitting left of the hidden
        /// divider; restore it to the visible section.
        case iibarIcon(MenuBarItem)
    ...
    /// of the three iiBar control items.
    ///
    ...
    /// that drag the iiBar icon to the user's configured anchor on every
    /// cache cycle. Visible-control-item exclusion was the omission
    /// that caused the field-reported "iiBar icon keeps moving" bug.
    ///
    ...
    /// hosts under Control Center that iiBar cannot yet attribute to their
    ...
    /// (1) iiBar visible-control icon recovery, (2) non-hideable system
    ...
    static nonisolated func planLeftmostMove(
        ...
        // Items sitting left of the hidden divider. The iiBar icon is a
        ...
        // Path 1: iiBar icon.
        if let iibarIcon = leftmostItems.first(where: { $0.tag == .visibleControlItem }) {
            return .iibarIcon(iibarIcon)
        }
    ...
    /// mis-positions the control items (the iiBar visible icon jumps to the far
    ...
    static nonisolated func planNotchOverflow(
        ...
        // always-visible iiBar icon to the leftmost slot on every overflow even
