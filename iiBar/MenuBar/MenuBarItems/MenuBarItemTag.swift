...
struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    var isSystemItem: Bool {
        switch namespace {
        case .controlCenter, .systemUIServer, .textInputMenuAgent, .weather, .passwords, .screenCaptureUI, .ssMenuAgent, .iibar, .gamePolicyAgent:
            return true
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .iibar, title: identifier.rawValue, instanceIndex: 0)
    }
}
...
extension MenuBarItemTag.Namespace {
    /// The namespace for the "iiBar" process.
    static let iibar = string(Constants.bundleIdentifier)
