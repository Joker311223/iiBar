...
final class MenuBarItemManager: ObservableObject {
    ...
    ///     (iiBar chevron); its position within the visible section is
    ...
    func computeSectionOrder(from cache: ItemCache) -> [String: [String]] {
        ...
        // are the typical case. The visibleControlItem (iiBar chevron) is
        // also persisted so its user-chosen position within the visible
        // section survives iiBar restarts: without it, savedSectionOrder
    ...
    /// Records that a move operation occurred outside of iiBar's own `move()` function
...
extension MenuBarItemManager {
    ...
    /// source PID and are not iiBar control items.
    ///
    ...
    /// Whether bundleID owns a menu bar item iiBar already tracks: an entry
...
extension MenuBarItemManager {
    ...
    func activate(item: MenuBarItem, on displayID: CGDirectDisplayID?) async {
        if Bridging.isWindowOnScreen(item.windowID) {
            // Electron/Chromium tray items (e.g. Claude) ignore iiBar's synthetic
...
extension MenuBarItemManager {
    ...
    func temporarilyShow(item: MenuBarItem, clickingWith mouseButton: CGMouseButton, on displayID: CGDirectDisplayID? = nil, fastPath: Bool = false) async -> TemporaryShowResult {
        ...
        // Prefer inserting to the left of the iiBar/visible control item so the icon appears
...
extension MenuBarItemManager {
    ...
    private func relocateNewLeftmostItems(
        ...
        switch decision {
        case let .iibarIcon(iibarIcon):
            MenuBarItemManager.diagLog.info("Relocating iiBar icon \(iibarIcon.logString) to visible section")
            do {
                try await move(
                    item: iibarIcon,
                ...
                MenuBarItemManager.diagLog.error("Failed to relocate iiBar icon \(iibarIcon.logString): \(error)")
                return false
            }
            return true
...
extension MenuBarItemManager {
    ...
    /// movable, hideable items (except the iiBar icon) to the
    /// Hidden section.
    ///
    ...
    private func resetLayoutWithControlItems(controlItems: ControlItemPair, items: [MenuBarItem]) async throws -> Int {
        ...
        func movePass(_ items: [MenuBarItem], anchor: MenuBarItem) async -> Int {
            ...
            for item in items {
                if item.tag == .visibleControlItem {
                    continue // Keep the iiBar icon in the visible section if enabled.
                }
    ...
    /// quit apps still appear in Settings Layout, iiBar Bar, and Search
    ...
    func applyProfileLayout(
        ...
        // PID are widgets macOS hosts under Control Center that iiBar cannot yet
        ...
        // items that won't fit into the hidden section. The iiBar visible
           ...
           let notch = screen.frameOfNotch
        {
            ...
            // Find the iiBar visible control icon, which must always stay visible.
            let visibleCtrlUID = items.first(where: { $0.tag == .visibleControlItem })?.uniqueIdentifier
    ...
    func applySavedLayout(
        ...
        // Each guard logs a distinct reason so a "iiBar stopped
        ...
        // geometry and mis-positions the iiBar visible icon to the far left (the
