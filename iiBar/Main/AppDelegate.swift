...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationWillFinishLaunching(_: Notification) {
        ...
        // Register iibar:// URL events early so external tools (e.g. Raycast)
        // can trigger actions even when iiBar is not currently in the foreground;
    ...
    /// Handles `kAEGetURL` Apple Events and forwards `iibar://` URLs to `handleURL(_:senderBundleId:)`.
    ...
    /// Dispatches an incoming `iibar://` URL to the appropriate action.
    ///
    /// Supported Action URLs:
    /// - `iibar://toggle-hidden` — toggle the hidden menu bar section
    /// - `iibar://toggle-always-hidden` — toggle the always-hidden menu bar section
    /// - `iibar://search` — open the menu bar item search panel
    /// - `iibar://toggle-iibarbar` — toggle the IceBar on the active display
    /// - `iibar://toggle-application-menus` — toggle application menus
    /// - `iibar://open-settings` — open the iiBar settings window
    ///
    /// Supported Settings URLs (requires whitelist authorization):
    /// - `iibar://set?key=X&value=Y` — set a boolean setting
    /// - `iibar://toggle?key=X` — toggle a boolean setting
    private func handleURL(_ url: URL, senderBundleId: String? = nil) {
        ...
        case "open-settings":
            openSettingsWindow()
        default:
            appState.diagLog.warning("Received unrecognized iibar:// URL: \(url.absoluteString)")
        }
    }
    ...
    /// Handles iibar://set?key=X&value=Y URL.
    ...
    /// Handles iibar://toggle?key=X URL.
    ...
    /// Handles iibar://get?key=X&callback=Y URLs.
