...
extension HIDEventManager {
    ...
    private func handleShowOnClick(appState: AppState, screen: NSScreen, clickLocation: CGPoint, modifierFlags: NSEvent.ModifierFlags, isDoubleClick: Bool = false) {
        ...
        // on a widget's icon don't also trigger iiBar's show-on-click reveal.
        ...
        if isDoubleClick {
            ...
            {
                // If the always-hidden section is currently showing via the iiBar
                // Bar, a plain click should close it rather than switch the iiBar
    ...
    private func handleSmartRehide(
        ...
        // Only continue if the click is not inside the iiBar Bar, at
    ...
    /// using the system-wide accessibility hit-test. Excludes iiBar's own
    ...
    /// role). When the helper returns true, iiBar defers to the widget under
    ...
    /// Returns whether any non-iiBar window at the pop-up-menu window level is
    ...
    /// overlay, which is needed to avoid showing iiBar's secondary context
    ...
    ///
    /// After a profile change with iiBar Bar active, the Window Server tracks
...
extension HIDEventManager {
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the iiBar Bar panel.
    func isMouseInsideIceBar(appState: AppState) -> Bool {
        ...
        // moves their mouse outside of the iiBar Bar.
